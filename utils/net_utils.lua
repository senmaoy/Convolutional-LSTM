...
function net_utils.build_cnn(cnn, opt)
  ...
  for i = 1, layer_num do  --change the layer_num to get annotations!!!!!!
    local layer = cnn:get(i)
    --print(layer.weight[1][1][1])
   -- print(layer.weight:dim())
    ...
    --if i == 25 then 
    -- local dilatedconv = nn.SpatialDilatedConvolution(512,512,3,3,1,1,1,1,1,1)
    -- dilatedconv.weight:copy(layer.weight)
    -- dilatedconv.bias:copy(layer.bias)
    -- cnn_part:add(dilatedconv)
  --  end
    if i ~= 24 then cnn_part:add(layer) end
    --if i ~= 24 and i ~= 25 then cnn_part:add(layer) end
  end
  --local dilatedfcn6 = nn.SpatialDilatedConvolution(512,4096,7,7,1,1,3,3,1,1)
  --local layer  = cnn:get(33)
  --dilatedfcn6.weight = layer.weight:view(4096,512,7,7):contiguous()
  --dilatedfcn6.bias:zero()-- here remains  a problem
  --cnn_part:add(dilatedfcn6)
...
function layer:updateOutput(input)
  print(string.format('%d',input:size(1)))
  print(string.format('%d',input:size(2)))
  print(string.format('%d',input:size(3)))
  ...
  local reshapedinput = torch.reshape(input, torch.LongStorage{input:size(1), d,input:size(3)*input:size(4)})
  self.output:resize(input:size(1)*self.n, d,input:size(3)*input:size(4))
  for k=1,input:size(1) do
    local j = (k-1)*self.n+1
    self.output[{ {j,j+self.n-1} }] = reshapedinput[{ {k,k}, {}, {} }]:expand(self.n, d,input:size(3)*input:size(4)) -- copy over
  end
  return self.output
end
function layer:updateGradInput(input, gradOutput)
  ...
  -- the cnn part is fixed so the expander do not need to feed backward. this part can be 
