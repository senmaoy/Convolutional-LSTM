...
cmd:option('-gpuid', 1, 'which gpu to use. -1 = use CPU')   --1 for small GPU
cmd:text()
...
for i = 1,12 do 
  jointskernel2:add(nn.SpatialConvolution(128,1,1,1,1,1,0,0))
end
--
...
grad_params4:zero()
 ...
 local responseMaprelu1 = relu:forward(responseMap[1])
  ...
  local treemapss = posetree:forwardup()
  local treemaps = {}
  for i = 2,12 do
 ...
 print(#grad_params3)
 local dtreejointmaps = {}
 for i = 2,12 do
  dtreejointmaps[i-1] = djointmaps[i]
 end
 
 local dtreemaps = posetree:backwardup(dtreejointmaps)
 local dtreemapss = {}
 for i = 2,12 do
  dtreemapss[i] = dtreemaps[i-1]
 end
  dtreemapss[1] = djointmaps[1]
